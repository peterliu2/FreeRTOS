...
{
    if (Xclock==TIM_EXTERNAL) TIMx->CR1|=TIM_ECKEN_Mask;
    else TIMx->CR1&=~TIM_ECKEN_Mask;
}
...
{
    if (Xedge == TIM_RISING) TIMx->CR1 |= TIM_EXEDG_Mask;
    else TIMx->CR1 &= ~TIM_EXEDG_Mask;
}
...
{
    if (NewState == ENABLE) TIMx->CR2 |= New_IT;
    else TIMx->CR2 &= ~New_IT;
}
