...
{
    if (NewState==ENABLE) UARTx->IER|=UART_Flag;
    else UARTx->IER&=~UART_Flag;
}
...
{
    if (NewState==ENABLE) UARTx->CR|=0x0400;
    else UARTx->CR&=~0x0400;
}
...
{
    if (FIFO==UART_RxFIFO) UARTx->RxRSTR=0xFFFF;
    else UARTx->TxRSTR=0xFFFF;
}
...
{
    if (NewState==ENABLE) UARTx->CR|=0x0040;
    else UARTx->CR&=~0x0040;
}
...
{
    if (NewState==ENABLE) UARTx->CR|=0x0100;
    else UARTx->CR&=~0x0100;
}
...
{
    if (NewState==ENABLE) UARTx->CR|=0x0080;
    else UARTx->CR&=~0x0080;
}
