...
static void NEAR _GET_PAGE_REG(void)   /*lint -esym(528, _GET_PAGE_REG) used in asm code */
{
    asm {
        ...
        BLO  L_EPAGE              ;
        EPAGE accesses are possible
#else
        BLO  L_NOPAGE             ;
        no paged memory below accesses
#endif
...
#else
        BHI  L_NOPAGE             ;
        no paged memory above accesses
#endif
        FOUND_DPAGE:
...
void NEAR _LOAD_FAR_16(void)
{
    ...
    asm {
        ...
        set page register
        LDY   0,Y            ;
        actual load, overwrites address
        STAA  0,X            ;
        restore page register
        PULA                 ;
        restore A register
...
void NEAR _LOAD_FAR_24(void)
{
    ...
    asm {
        ...
        set page register
        LDAB  0,Y            ;
        actual load, overwrites page of address
        LDY   1,Y            ;
        actual load, overwrites offset of address
        STAA  0,X            ;
        restore page register
        PULA                 ;
        restore A register
...
void NEAR _STORE_FAR_32(void)
{
    ...
    asm {
        ...
        MOVW   0,SP, 2,Y      ;
        store the value passed in D (low word)
        STAA   0,X            ;
        restore page register
        PULD                  ;
        restore A register
...
void NEAR _FAR_COPY(void)
{
    ...
    asm {
        DEX                   ;
        source addr-=1, because loop counter ends at 1
        PSHX                  ;
        save source offset
        PSHD                  ;
        save both pages
        DEY                   ;
        destination addr-=1, because loop counter ends at 1
        PSHY                  ;
        save destination offset
        LDX     8,SP          ;
        load counter, assuming counter > 0

        loop:
        LDD     4,SP          ;
        load source offset
        LEAY    D,X           ;
        calcutate actual source address
        LDAB    2,SP          ;
        load source page
        __PIC_JSR (_LOAD_FAR_8);
        load 1 source byte
        PSHB                  ;
        save value
        LDD     0+1,SP        ;
        ...
        LEAY    D,X           ;
        calcutate acual destination address
        PULA                  ;
        restore value
        LDAB    3,SP          ;
        load destination page
        __PIC_JSR (_STORE_FAR_8);
        store one byte
        DEX
        BNE     loop
        LDX     6,SP          ;
    ...
    asm {
        ...
        LDAA  2,SP+            ;
        restore old page value and release stack
        STAA  PAGE_ADDR        ;
        store it into page register
        LDX   4,SP+            ;
        release stack and load return address
        JMP   0,X              ;
        return
    }
#endif
