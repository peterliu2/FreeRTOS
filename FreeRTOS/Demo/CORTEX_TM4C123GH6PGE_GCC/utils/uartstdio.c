...
// semaphore to wake command line process task.
SemaphoreHandle_t xUartRxCommandCompleteSemaphore = NULL;
...
static void UARTPrimeTransmit( uint32_t ui32Base )
{
    ...
    if( !TX_BUFFER_EMPTY ) {
		...
		uint32_t ulStoreInterrputMask = portSET_INTERRUPT_MASK_FROM_ISR();

        // Yes - take some characters out of the transmit buffer and feed
        // them to the UART transmit FIFO.
        while( MAP_UARTSpaceAvail( ui32Base ) && !TX_BUFFER_EMPTY ) {
            MAP_UARTCharPutNonBlocking( ui32Base, g_pcUARTTxBuffer[ g_ui32UARTTxReadIndex ] );
            ADVANCE_TX_BUFFER_INDEX( g_ui32UARTTxReadIndex );
        }

        // Reenable the UART interrupt.
        //MAP_IntEnable( g_ui32UARTInt[ g_ui32PortNum ] );
		portCLEAR_INTERRUPT_MASK_FROM_ISR( ulStoreInterrputMask );
    }
}
#endif
...
{
    // Check the arguments.
    configASSERT( ( ui32PortNum == 0 ) || ( ui32PortNum == 1 ) || ( ui32PortNum == 2 ) );

#ifdef UART_BUFFERED
    // In buffered mode, we only allow a single instance to be opened.
    configASSERT( g_ui32Base == 0 );
#endif
    ...
    MAP_UARTConfigSetExpClk( g_ui32Base, ui32SrcClock, ui32Baud, ( UART_CONFIG_PAR_NONE | UART_CONFIG_STOP_ONE | UART_CONFIG_WLEN_8 ) );

#ifdef UART_BUFFERED
    // create the semaphore used to wake command line process task from Uart ISR.
    vSemaphoreCreateBinary( xUartRxCommandCompleteSemaphore );
    xSemaphoreTake( xUartRxCommandCompleteSemaphore, 0 );
	...
	// setting UART interrupt priority
	IntPrioritySet( g_ui32UARTInt[ ui32PortNum ], configKERNEL_INTERRUPT_PRIORITY );
...
int UARTgets( char *pcBuf, uint32_t ui32Len )
{
    ...
    // Check the arguments.
    configASSERT( pcBuf != 0 );
    configASSERT( ui32Len != 0 );
    configASSERT( g_ui32Base != 0 );

    // wait for Uart ISR to wake this task */
    while ( xSemaphoreTake( xUartRxCommandCompleteSemaphore, portMAX_DELAY ) != pdPASS );
    ...
    // Add a newline character to inducate the string is terminated.
    pcBuf[ ui32Count++ ] = '\n';

    // Return the count of in the buffer, counting the trailing '\n'.
    return( ui32Count );
#else
    uint32_t ui32Count = 0;
    int8_t cChar;
...
int UARTPeek( unsigned char ucChar )
{
    ...
    for( iCount = 0; iCount < iAvail; iCount++ ) {
        if( g_pcUARTRxBuffer[ui32ReadIndex] == ucChar ) {
            // We found it so return the count.
            return( iCount );
        } else {
...
void UARTFlushRx(void)
{
    ...
    //uint32_t ui32Int = MAP_IntMasterDisable();
    portENTER_CRITICAL();
...
void UART0_IRQHandler( void )
{
    ...
    if( ui32Ints & ( UART_INT_RX | UART_INT_RT ) ) {
        ...
        while( MAP_UARTCharsAvail( g_ui32Base ) ) {
            // Read a character
            cChar = ( char )( MAP_UARTCharGetNonBlocking( g_ui32Base ) & 0xFF);
            ...
            if( !g_bDisableEcho ) {
                ...
                if( ( cChar == '\r' ) || ( cChar == '\n' ) || ( cChar == 0x1b ) ) {
                    ...
                    // put a LF in the receive buffer as a marker telling
                    // UARTgets() where the line ends.  We also send an
                    // additional CR to ensure that the local terminal echo
                    // receives both CR and LF.
                    cChar = '\n';
                    UARTwrite( "\r", 1 );

                    // Wake the command line process.
                    portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
                    xSemaphoreGiveFromISR( xUartRxCommandCompleteSemaphore, & xHigherPriorityTaskWoken );
